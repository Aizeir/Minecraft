...
void main() {
    vec3 diffuse = texture2D(material.diffuse, uvs).rgb;
    ...
    if (light.vector.w == 0.0) {
        ...
        light_dir = normalize(light.vector.xyz - frag_pos); // towards LIGHT
    }
    ...
    vec3 specular = texture2D(material.specular, uvs).rgb;
    float specular_value = pow(max(dot(view_dir, reflect_dir), 0.0), material.shininess);
    vec3 specular_color = light.specular * (specular_value * specular);
